		...
		while (lcv < 8) {
			...
			try {
				...
				//Straight right check
					...
					//Straight left check
					if (king.validCoordinate(dummyBoard, king.getHorz() - 1, king.getVert())) {
						currentValue += this.calculateSingleMove(dummyBoard, king, pieceList, king.getHorz() - 1, king.getVert());
						if (currentValue > bestValue) {
							bestValue = currentValue;
							bestHorz = king.getHorz() - 1;
							bestVert = king.getVert();
							lcv++;
						}
						else {
							lcv++;
				...
				//Straight forward check
					...
					//Straight forward check
					if (king.validCoordinate(dummyBoard, king.getHorz(), king.getVert() + 1)) {
						currentValue += this.calculateSingleMove(dummyBoard, king, pieceList, king.getHorz(), king.getVert() + 1);
						if (currentValue > bestValue) {
							bestValue = currentValue;
							bestHorz = king.getHorz();
							bestVert = king.getVert() + 1;
							lcv++;
						}
						else {
							lcv++;
				...
				//Right diag check
					...
					if (king.validCoordinate(dummyBoard, king.getHorz() - 1, king.getVert() - 1)) {
						currentValue += this.calculateSingleMove(dummyBoard, king, pieceList, king.getHorz() - 1, king.getVert() - 1);
						if (currentValue > bestValue) {
							bestValue = currentValue;
							bestHorz = king.getHorz() - 1;
							bestVert = king.getVert() - 1;
		...
		while (lcv < 8) {
			...
			try {
				...
				/*
					...
					//Left Lower tall; check for valid coordinate
					if (knight.validCoordinate(dummyBoard, knight.getHorz() - 1, knight.getVert() + 2)){
						//If coordinate is valid, calculate move and store in currentValue
						currentValue = this.calculateSingleMove(dummyBoard, knight, pieceList, knight.getHorz() - 1, knight.getVert() + 2);
						//Check if values have changed; if changed, change coordinates and value
						if (currentValue > bestValue) {
							bestValue = currentValue;
							bestHorz = knight.getHorz() - 1;
							bestVert = knight.getVert() + 2;
							lcv++;
						}
						else if (currentValue == bestValue) {
							double current, best;
							...
							//Check both current and tested coordinates for move values
							this.duplicateBoard(board, dummyBoard);
							current = dummyBoard[knight.getHorz() - 1][knight.getVert() + 2].threatenedPieces(dummyBoard) 
									  + dummyBoard[knight.getHorz() - 1][knight.getVert() + 2].threatenedSquares(dummyBoard);
							//Redup board for moving and check best coordinate
							this.duplicateBoard(board, dummyBoard);
							best = dummyBoard[bestHorz][bestVert].threatenedPieces(dummyBoard) 
								    + dummyBoard[bestHorz][bestVert].threatenedSquares(dummyBoard);
					...
					//Left upper long; check for valid coordinate
					if (knight.validCoordinate(dummyBoard, knight.getHorz() - 2, knight.getVert() - 1)){
						//If coordinate is valid, calculate move and store in currentValue
						currentValue = this.calculateSingleMove(dummyBoard, knight, pieceList, knight.getHorz() - 2, knight.getVert() - 1);
						//Check if values have changed; if changed, change coordinates and value
						if (currentValue > bestValue) {
							bestValue = currentValue;
							bestHorz = knight.getHorz() - 2;
							bestVert = knight.getVert() - 1;
							lcv++;
						}
						else if (currentValue == bestValue) {
							double current, best;
							...
							//Check both current and tested coordinates for move values
							this.duplicateBoard(board, dummyBoard);
							current = dummyBoard[knight.getHorz() - 2][knight.getVert() - 1].threatenedPieces(dummyBoard) 
									  + dummyBoard[knight.getHorz() - 2][knight.getVert() - 1].threatenedSquares(dummyBoard);
							//Redup board for moving and check best coordinate
							this.duplicateBoard(board, dummyBoard);
							best = dummyBoard[bestHorz][bestVert].threatenedPieces(dummyBoard) 
								    + dummyBoard[bestHorz][bestVert].threatenedSquares(dummyBoard);
					...
					//Left lower long; check for valid coordinate
					if (knight.validCoordinate(dummyBoard, knight.getHorz() - 2, knight.getVert() + 1)){
						//If coordinate is valid, calculate move and store in currentValue
						currentValue = this.calculateSingleMove(dummyBoard, knight, pieceList, knight.getHorz() - 2, knight.getVert() + 1);
						//Check if values have changed; if changed, change coordinates and value
					...
					//Right upper tall; check for valid coordinate
					if (knight.validCoordinate(dummyBoard, knight.getHorz() + 1, knight.getVert() - 2)){
						//If coordinate is valid, calculate move and store in currentValue
						currentValue = this.calculateSingleMove(dummyBoard, knight, pieceList, knight.getHorz() + 1, knight.getVert() - 2);
						//Check if values have changed; if changed, change coordinates and value
					...
					//Right Lower tall; check for valid coordinate
					if (knight.validCoordinate(dummyBoard, knight.getHorz() + 1, knight.getVert() + 2)){
						//If coordinate is valid, calculate move and store in currentValue
						currentValue = this.calculateSingleMove(dummyBoard, knight, pieceList, knight.getHorz() + 1, knight.getVert() + 2);
						//Check if values have changed; if changed, change coordinates and value
						...
						//Base which move is better based on number of squares attack
						else if (currentValue == bestValue) {
							//Determine which coordinate is closer to center; go with more central square
							double testCurrentHorz, testCurrentVert, testBestHorz, testBestVert, totalTestBest, totalTestCurrent;
							testCurrentHorz = Math.abs(4.5 - (knight.getHorz() + 1));
							testBestHorz = Math.abs(4.5 - bestHorz);
							
							testCurrentVert = Math.abs(4.5 - (knight.getVert() + 2));
							testBestVert = Math.abs(4.5 - bestVert);
							
							totalTestBest = testBestHorz + testBestVert;
							totalTestCurrent = testCurrentHorz + testCurrentVert;
							
							//If both test values are smaller, use current coordinates
							if (totalTestCurrent < totalTestBest){
								bestHorz = knight.getHorz() + 1;
								bestVert = knight.getVert() + 2;
							}
							lcv++;
					...
					//Right upper long; check for valid coordinate
					if (knight.validCoordinate(dummyBoard, knight.getHorz() + 2, knight.getVert() - 1)){
						//If coordinate is valid, calculate move and store in currentValue
						currentValue = this.calculateSingleMove(dummyBoard, knight, pieceList, knight.getHorz() + 2, knight.getVert() - 1);
						//Check if values have changed; if changed, change coordinates and value
					...
					//Right lower long; check for valid coordinate
					if (knight.validCoordinate(dummyBoard, knight.getHorz() + 2, knight.getVert() + 1)){
						//If coordinate is valid, calculate move and store in currentValue
						currentValue = this.calculateSingleMove(dummyBoard, knight, pieceList, knight.getHorz() + 2, knight.getVert() + 1);
						//Check if values have changed; if changed, change coordinates and value
